...
function _broadcastInfo() {
	...
	_subnet = "/24"
	_ipSubnet = $(ip)$(_subnet)
	
	_nmapVulns = "/usr/share/nmap/scripts"
...
# nmap -D = -D <decoy1>[,<decoy2>][,ME][,...]
# -S
# -sn = no port scan
# -sx = bruteforce
# - e = use specified NIC
# --data-lenght = append random data to packets
# --backsum = send packets with bogus checksum
#

function grabNetworkStats() {
	echo "[!] Grabbing Network Stats on \n $_ip "
	echo "[!} Enter IP for scan. " read nmapIP
	echo "[!] IP + SUBNET \n $(_ip_addr) $(_ipSubnet) "
		...
		git clone "https://github.com/vulnerscom/nmap-vulners" && cd ./nmap-vulners
		...
		# -PE, -PP, and -PM
		sudo  proxychains nmap -sn --packet-trace  -PA -S 192.160.50.1 --spoof-mac $_mac $_ip_addr  --reason #ack ping
		sudo proxychains nmap -sn --packet-trace -PE -R -v -S 192.160.50.1 --spoof-mac $_mac $_ip_addr --reason
		sudo proxychains nmap -sn --packet-trace -PR -S 192.160.50.1 --spoof-mac $_mac $_ip_addr --reason
		sudo proxychains nmap -n --packet-trace -sn -S 192.160.50.1 --send-ip $_ip --spoof-mac $_mac $_ip_addr --reason
		sudo proxychains nmap -sn --packet-trace -S 192.160.50.1 -PS80 -R -v --spoof-mac $_mac $_ipaddr --reason ## TCP Scan
		sudo proxychains nmap -n -sn --packet-trace  -S 192.160.50.1 --send-ip $_ip_addr --reason #arp scan
		sudo proxychains nmap -n -sn --packet-trace -S 192.160.50.1  -PR --spoof-mac $_mac --packet-trace --send-eth $_ip_addr --reason
		sudo proxychains nmap -sN --packet-trace -S 192.160.50.1 --traceroute  $_ipSubnet --reason # nmap traceroute
		sudo proxychains nmap -sL --packet-trace $(_ipSubnet) --reason
		
		sudo procychains nmap -sN -sV $(_ip_addr)  # nmap direct port scan. (not ideal for subnets)
		sudo proxychains nmap -O $(_ip_addr) --osscan-guess # nmap oscan, with guess
		sudo proxychains nmap -sV $(_ipSubnet) --version-intensity=9 ## nmap intense OS Scan
		
		## NMAP VULN SCRIPT
		sudo proxychains nmap -sV -A --script=vulners $(_ipSubnet) --version-intensity=9 --reason ## nmap OS intense scan, with vuln scripts enabled
		sudo proxychains nmap -sV -sS -Pn -A -V $(_ipSubnet) --reason
		sudo proxychains nmap --script nmap-vulners/vullnscan/ -sV -Pn -A -V $(_ipSubnet)
		
		sudo proxychains nmap -sV $(_ip_addr) # nmap service port guess, direct
		sudo proxychains nmap -sV $(_ipSubnet) # nmap service port guess, on the subnet
		
		## START OF NMAP BRUTEFORCE ##
		sudo proxychains nmap -sX $()
